...
"ilovemoney" relies on a configuration file. If you run the
...
By default, Ilovemoney loads its configuration from `/etc/ilovemoney/ilovemoney.cfg`.

If you need to load the configuration from a custom path, you can define
the `ILOVEMONEY_SETTINGS_FILE_PATH` environment variable with the path
to the configuration file. For instance :

    export ILOVEMONEY_SETTINGS_FILE_PATH="/path/to/your/conf/file.cfg"

The path should be absolute. A relative path will be interpreted as
being inside `/etc/ilovemoney/`.
...
-   **Default value:** `sqlite:////tmp/ilovemoney.db`
-   **Production value:** Set it to some path on your disk. Typically
    `sqlite:////home/ilovemoney/ilovemoney.db`. Do *not* store it under
...
-   **Production value:** `ilovemoney conf-example ilovemoney.cfg`
...
-   **Production value:** To generate the proper password HASH, use
    `ilovemoney generate_password_hash` and copy the output into the
...
By default, ilovemoney will be served at domain root (e.g:
...
of the server running ilovemoney, which may or may not be what you want.
...
-   **Default value:** `"I Love Money"` (empty string)
-   **Production value:** The name of your choosing


## Configuring email sending

By default, Ilovemoney sends emails using a local SMTP server, but it's
