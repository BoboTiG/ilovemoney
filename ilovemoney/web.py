...
from ilovemoney.emails import send_creation_email
from ilovemoney.forms import (
...
from ilovemoney.history import get_history, get_history_queries, purge_history
from ilovemoney.models import Bill, BillType, LoggingMode, Person, Project, db
...
def migrate_session(endpoint, values):
    if "projects" in session and isinstance(session["projects"], list):
        # Migrate https://github.com/BoboTiG/ilovemoney/pull/1082
...
def invite():
    ...
    invite_link = urlunparse(urlparse(invite_link)._replace(scheme="ilovemoney"))
