...
from ilovemoney.utils import list_of_dicts2csv, list_of_dicts2json
...
class CommonTestCase(object):
    ...
    class Import(IlovemoneyTestCase):
        def setUp(self):
            super().setUp()
            self.data = [
                {
        ...
        def test_import_no_currency_in_empty_project_with_currency(self):
            # Import JSON without currencies (from ilovemoney < 5) in an empty
...
class TestExport(IlovemoneyTestCase):
