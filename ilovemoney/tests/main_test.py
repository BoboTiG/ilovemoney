...
from ilovemoney import models
from ilovemoney.currency_convertor import CurrencyConverter
from ilovemoney.manage import delete_project, generate_config, password_hash
from ilovemoney.run import load_configuration
from ilovemoney.tests.common.ilovemoney_testcase import BaseTestCase, IlovemoneyTestCase

# Unset configuration file env var if previously set
os.environ.pop("ILOVEMONEY_SETTINGS_FILE_PATH", None)
...
class TestConfiguration(BaseTestCase):
    ...
    def test_env_var_configuration_file(self):
        ...
        os.environ["ILOVEMONEY_SETTINGS_FILE_PATH"] = os.path.join(
            __HERE__, "ilovemoney_envvar.cfg"
        )
        load_configuration(self.app)
        assert self.app.config["SECRET_KEY"] == "lalatra"

        # Test that the specified configuration file is loaded
        # even if the default configuration file ilovemoney.cfg exists
...
class TestServer(IlovemoneyTestCase):
    def test_homepage(self):
        # See https://github.com/BoboTiG/ilovemoney/pull/358
...
class TestModels(IlovemoneyTestCase):
...
class TestEmailFailure(IlovemoneyTestCase):
...
class TestCaptcha(IlovemoneyTestCase):
    ENABLE_CAPTCHA = True
    ...
    def test_project_creation_with_captcha_case_insensitive(self):
        ...
        with patch("ilovemoney.forms._", new=lambda x: "ÉÙÜẞ"), self.client as c:
            c.post(
                "/create",
                data={
                    "name": "raclette party",
