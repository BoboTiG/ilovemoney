...
from ilovemoney import default_settings
from ilovemoney.api.v1 import api as apiv1
from ilovemoney.currency_convertor import CurrencyConverter
from ilovemoney.models import db
from ilovemoney.utils import (
    IhmJSONEncoder,
    PrefixedWSGI,
    em_surround,
    limiter,
...
from ilovemoney.web import main as web_interface
...
def setup_database(app):
    ...
    def _pre_alembic_db():
        """Checks if we are migrating from a pre-alembic ilovemoney"""
...
def load_configuration(app, configuration=None):
    ...
    - Use the ILOVEMONEY_SETTINGS_FILE_PATH env var if defined ;
    - If not, use /etc/ilovemoney/ilovemoney.cfg ;
    - Otherwise, load the default settings.
    """

    env_var_config = os.environ.get("ILOVEMONEY_SETTINGS_FILE_PATH")
    app.config.from_object("ilovemoney.default_settings")
    if configuration:
        app.config.from_object(configuration)
    elif env_var_config:
        app.config.from_pyfile(env_var_config)
    else:
        app.config.from_pyfile("ilovemoney.cfg", silent=True)
...
def validate_configuration(app):
    ...
    if "pbkdf2:" not in app.config["ADMIN_PASSWORD"] and app.config["ADMIN_PASSWORD"]:
        # Since 2.0
        warnings.warn(
            "The way Ilovemoney stores your ADMIN_PASSWORD has changed. You are using an unhashed"
            + " ADMIN_PASSWORD, which is not supported anymore and won't let you access your admin"
            + " endpoints. Please use the command 'ilovemoney generate_password_hash'"
...
def create_app(
    configuration=None, instance_path="/etc/ilovemoney", instance_relative_config=True
):
    app = Flask(
        __name__,
